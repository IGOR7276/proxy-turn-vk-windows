...
func (a *App) Deploy(p DeployParams) error {
	...
	if len(deployScript) == 0 || len(serverBinary) == 0 {
		emit("⚠ deploy-скрипт или бинарь не встроены (см. backend.Init)")
		return fmt.Errorf("deploy assets not embedded")
	}
...
func (a *App) Undeploy(p DeployParams) error {
	...
	if len(deployScript) == 0 {
		emit("⚠ deploy-скрипт не встроен")
		return fmt.Errorf("deploy assets not embedded")
	}
