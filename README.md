...
WDTT также доступен как **нативный Go-клиент для Windows**. Он не требует установки Android-приложения — запускается из командной строки или через встроенный веб-интерфейс.

### Использование

```bash
# CLI режим
go_client\wdtt-client.exe -peer <VPS_IP>:56000 -vk <VK_HASH> -password <PASSWORD> -n 36 -windows-wg

# Веб-интерфейс (откроется в браузере)
go_client\wdtt-client.exe -ui
```
...
Требуется Go 1.21+ и `wintun.dll` (автоматически копируется из WireGuard for Windows).
